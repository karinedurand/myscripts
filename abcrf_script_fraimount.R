...
# Visualize numerical results for the global prior error rates and the matrix of confusion (i.e. prior error rates detailed for
each scenario)
...
Script #2: this second R script allows to running successively several ABC-RF model choice analyses starting from
30the same DIYABC v2.1.0 reference table and this for single dataset of observed summary statistics
# R SCRIPT#2 to run successively SEVERAL ABC-RF model choice analyses starting from the SAME reference table
generated by the DIYABC v2.1.0 package (.bin format) and from a single dataset of observed summary statistics
# Date: 01/09/2016 version 1.0.
# Licence: GPL2
# Authors: Arnaud Estoup, Jean-Michel Marin, Julien Foucaud, Alex Dehne-Garcia, and Antoine Fraimout
# Note: the example below is based on a DIYABC reference table named "reftable.bin" including 4 scenarios (models)
...
"statobs.txt" (the latter including the summary statistics of the observed dataset)
# Add in the working directory the DIYABC v2.1.0 core executable file which can be downloaded at
http://www1.montpellier.inra.fr/CBGP/diyabc/ for different operating systems (cf. downloadable file diyabc_core-2.1.0-
linWinOsXExe.zip).
# The name of the DIYABC 2.1 core executable files is:
# diyabc_core-2.1.0-win.exe for windows
# diyabc_core-2.1.0-OsX for mac
...
31# Outputs for each analysis are presented as following: "Nref best_scenario posterior_probability vote_scen1 vote_scen2
...
# Here statobs.txt file includes 3 rows corresponding to 3 vectors of observed summary statistics that will be analysed
serially.
...
T1<-Sys.time()
ncol=max(count.fields("reftable.txt",skip=2*nscenarios+3))
34toto=read.table("reftable.txt",skip=2*nscenarios+3,header=FALSE,fill=TRUE,row.names=NULL,col.names=paste("V",1:
ncol,sep=""),nrows=Nrefmax)
toto=toto[order(toto[,1]),]
rm(ncol)
import.reftable=function(nb.SS=nSS)
...
output <- output[-1,]
35colnames(output) <- c("Nref", "Statobs_row_ID", "Best_scenario", "Posterior_probability",
